...
if($check==''){
$array=array("status"=>"success", "id"=>"$idsql", "msg"=>"Đã Xoá Thành Công Cookie Die.",);
$json=json_encode($array);
echo $json;
$xoa="DELETE FROM `cookiefb` WHERE `cookiefb`.`id` = $idsql";
mysqli_query($ketnoi, $xoa);
exit();
}
}
$array=array("status"=>"error","msg"=>"Đã Hết Cookie Die.",);
$json=json_encode($array);
echo $json;
?>