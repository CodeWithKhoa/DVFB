...
echo $daucau.$luc."Nhập Id share: ".$vang;
$idshare=trim(fgets(STDIN));
$js=json_decode(dvfb('gettokenpage',null,null,$cookie),true);
$data=$js['message'];
if($js['status']=='error'){
    echo $js['message'];
    die;
...
banner();
echo $thanhngang;
echo $luc."Thành Công Đã Thấy ".$vang.$soluong.$luc." Page\n";
echo $thanhngang;
echo $luc.'Cấu Hình: '.$vang.$name.$luc.' Id: '.$vang.$id."\n";
echo $thanhngang;
$stt=0;
while (true){
    foreach ($js['message'] as $message) {
        $id = $message['id'];
        $name = $message['name'];
        $token = $message['access_token'];
        $share=json_decode(dvfb("shareao",$idshare,$token,$cookie),true);
        if($share['status']=='success'){
            $stt++;
            $tt = "\r\033[1;31m|\033[1;37m" . $stt . "\033[1;31m|\033[1;91m | \033[1;96m" . date("H:i:s") . "\033[1;91m |\033[1;93m".$id."\033[1;91m| \033[1;97m" . $name . "\033[1;91m | \033[1;97m" . $share['message'] . "\033[1;91m |\n";
